...
interface PostAdminSigninResult {
  code: string;
  accessToken: string;
}
...
const GlampingKing: React.FC = () => {
  const [accessToken, setAccessToken] = useRecoilState<string | null>(
    adminAccessTokenState,
  );
  const [adminId, setAdminId] = useRecoilState<string>(adminIdState);
  const [adminPw, setAdminPw] = useRecoilState<string>(adminPwState);
