...
import {
  postSearchOwnerCheckSms,
  postSearchOwnerEmail,
  PostSearchOwnerMailSms,
  PostSearchOwnerSendSms,
} from "../../apis/ceoapi";
import axios, { AxiosError } from "axios";

const CeoSearchEmail = () => {
  ...
  const handleSmsSubmit = async (
    ...
    try {
      ...
      // setLoading(true);
      const result = await PostSearchOwnerSendSms({ ceoPhone });
      ...
      if (result && result.data.code === "SU") {
        ...
        setIsSmsSent(true);
        setPhoneTimer(299);
      } else if (result && result.data.code === "IPH") {
        openModal({
          message: "전화번호 형식이 올바르지 않습니다.",
        });
      } else if (result && result.data.code === "NU") {
        openModal({
  ...
  const handleAuthNumberSubmit = async (
    ...
    try {
      const result = await postSearchOwnerCheckSms({ ceoPhone, authNumber });
      // console.log(result);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 핸드폰 인증코드 완료되었는지 확인
    try {
      const result = await postSearchOwnerEmail({ ceoName, ceoPhone });
      // console.log(result);
      if (result && result.data.code === "SU") {
        openModal({
